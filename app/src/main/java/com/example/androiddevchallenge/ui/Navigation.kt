...
import com.example.androiddevchallenge.util.getMutableStateOf
...
sealed class Screen(val id: ScreenName) {
    ...
    data class Detail(val dogId: String) : Screen(DETAIL)
}
...
private const val SIS_DOG_ID = "dog_id"
...
private fun Screen.toBundle(): Bundle {
    return bundleOf(SIS_NAME to id.name).also {
        ...
        if (this is Detail) {
            it.putString(SIS_DOG_ID, this.dogId)
        }
    }
}
...
private fun Bundle.toScreen(): Screen {
    ...
    return when (screenName) {
        HOME -> Home
        DETAIL -> {
            val dogId = getStringOrThrow(SIS_DOG_ID)
            Detail(dogId = dogId)
        }
    }
}
